#!/usr/bin/env bash
# Script to build and run OpenEMR Apache PHP-FPM in a Docker container (Linux arm64)
...
echo -e "${GREEN}============================================================================${NC}"
echo -e "${GREEN}Running OpenEMR with Apache PHP-FPM in Docker (Linux arm64)${NC}"
echo -e "${GREEN}============================================================================${NC}"

# Check for binaries
PHP_FPM=$(find "${LINUX_DIR}" -maxdepth 1 -type f -name "php-fpm-*-linux-arm64" | head -1)
...
docker build -t openemr-apache-fpm-arm64 \
