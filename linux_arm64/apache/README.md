# Apache Setup for OpenEMR on Linux (arm64)

This directory contains configuration files and scripts for running OpenEMR locally with Apache HTTP Server on Linux (arm64) using the static PHP CGI binary.
...
- **Static PHP CGI binary** - Uses the `php-cgi-*-linux-arm64` binary built by the Linux build script
...
![OpenEMR Setup Output](../../images/linux_arm64_apache_openemr_setup_output.png)
...
1. **Linux (arm64)** - This example is designed for Linux (arm64), such as Ubuntu or Debian on arm64 hardware.
...
3. **Built OpenEMR Binaries** - Run the build script first:
   ...
   ```
   This creates:
   - `php-cli-*-linux-arm64` - PHP CLI binary (for PHAR extraction)
   - `php-cgi-*-linux-arm64` - PHP CGI binary (used by Apache)
...
1. **Auto-detection**: The wrapper script (`php-wrapper.sh`) is designed to auto-detect the PHP CGI binary. It will look in the `linux_arm64` directory for a file matching `php-cgi-*-linux-arm64`.
