...
echo -e "${YELLOW}Step 3: Configuring Apache inside VM (this may take a few minutes)...${NC}"
EXPECT_SCRIPT=$(mktemp)
# Explicitly pass variables to avoid env issues
cat > "${EXPECT_SCRIPT}" << 'EXPECT_EOF'
set timeout -1
log_user 1
log_file "freebsd/expect-apache-run.log"
match_max 1000000
