#!/bin/sh
# PHP CGI Wrapper Script for Apache on Linux (amd64)
# 
...
if [ -z "${PHP_CGI_BINARY:-}" ]; then
    ...
    if [ -n "${DOCUMENT_ROOT:-}" ]; then
        ...
        # Find php-cgi-*-linux-amd64 or php-cgi-linux-amd64 in parent directory
        PHP_CGI_BINARY=$(find "${PARENT_DIR}" -maxdepth 1 -type f \( -name "php-cgi-*-linux-amd64" -o -name "php-cgi-linux-amd64" \) -perm /111 2>/dev/null | head -1)
    fi
    ...
    if [ -z "${PHP_CGI_BINARY:-}" ] || [ ! -f "${PHP_CGI_BINARY}" ]; then
        ...
        echo "Please set PHP_CGI_BINARY environment variable or ensure php-cgi-*-linux-amd64 exists in the parent directory"
        exit 1
    fi
fi
