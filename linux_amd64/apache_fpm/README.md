...
- **Static PHP FPM binary** - Uses the `php-fpm-*-linux-*` binary built by the Linux build script
...
cd ..
./run-web-server.sh
# (This extracts the PHAR to openemr-extracted/ and starts the built-in server, 
# you can stop the server after extraction)
```
...
![OpenEMR Setup Output](../../images/linux_amd64_apache_fpm_openemr_setup_output.png)
